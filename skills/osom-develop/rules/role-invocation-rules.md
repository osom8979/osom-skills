...
2. **문서 읽기 지시** — 역할에게 관련 규칙 문서(`STRUCTURE.md`의 `Structure doc`/`Code style doc` 값과 `Rules directory` 안의 관련 파일)를 먼저 읽으라고 지시.
3. **병렬 호출** — 같은 Phase 내 작업은 **하나의 메시지에서 동시에** 도구를 호출. 순차 호출은 Phase 분리 시에만 사용.
4. **결과 검증** — 각 Phase 완료 후 `COMMANDS.md`의 `Build/Check`로 빌드 확인.
