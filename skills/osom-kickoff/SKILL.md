...
3. `GUARDRAILS.md` — 제약
4. `ARCHITECTURE.md` — 런타임/레이어 (있다면)
5. `.claude/agents/` 또는 설치된 osom-skills 역할 스킬 목록 — 이번 작업에 사용 가능한 역할 파악
...
역할 목록은 프로젝트에 설치된 osom-skills 역할 스킬 중 이번 작업의 맥락에 맞는 것만 선택합니다. Phase는 **순차 실행**(Phase 내부 역할은 병렬)이며, 출력물이 다음 작업의 입력이 되는 역할은 다음 Phase로 분리합니다.
...
- 역할 스킬 분배 계획 수립
- 사용자 확인
- Phase별 순차 실행 (Phase 내부 역할 스킬은 병렬)
- 빌드 검증
- 문서 정합성 체크

를 수행합니다.

각 Phase 완료 시마다 빌드 검증 → 체크리스트 갱신 → 커밋을 수행합니다([커밋 순환 원칙](rules/commit-cycle-principle.md) 적용).
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  → /osom-develop 실행 중...
  → Phase 1 완료 → validate → checklist 갱신 → commit
  → Phase 2 완료 → validate → checklist 갱신 → commit
  → ...
```
